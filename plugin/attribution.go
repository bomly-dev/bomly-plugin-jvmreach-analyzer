...
// decide the root filter. See the comment on applyImportedArtifactSeeds:
// jvmreach's seed set is keyed by "group:artifact" with the classifier
// stripped, so it cannot separate two occurrences of one coordinate even when
// the graph holds them.
